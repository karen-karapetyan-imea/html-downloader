...
DEFAULT_PHAIDON_INDEX = "https://www.phaidon.com/sitemap.xml"
DEFAULT_UGALLERY_INDEX = "https://www.ugallery.com/sitemap.xml"
...
_UGALLERY_CHILD_RE = ("sitemap_products_", "sitemap_pages_")
...
def filter_ugallery_child_sitemaps(urls: Iterable[str]) -> list[str]:
    selected: list[str] = []
    for url in urls:
        lower = url.lower()
        if any(token in lower for token in _UGALLERY_CHILD_RE):
            selected.append(url)
    return selected
...
def filter_phaidon_child_sitemaps(urls: Iterable[str]) -> list[str]:
    """Keep default-locale Shopify product sitemaps only (skip /en-* locales)."""
    selected: list[str] = []
    for url in urls:
        lower = url.lower()
        if "sitemap_products_" not in lower:
            continue
        if "/en-" in lower:
            continue
        selected.append(url)
...
def fetch_ugallery_sitemap_entries(
    index_url: str = DEFAULT_UGALLERY_INDEX,
    *,
    concurrency: int = 8,
    proxy: dict[str, str] | None = None,
    fetch_bytes: FetchBytesFn | None = None,
) -> list[SitemapEntry]:
    """Fetch UGallery artwork/artist entity URLs from the Shopify XML sitemap index."""
    session: Any | None = None
    owned_session = False
    active_fetch = fetch_bytes
    ...
    try:
        index_xml = active_fetch(index_url)
        child_urls = filter_ugallery_child_sitemaps(parse_child_sitemap_locs(index_xml))
        LOGGER.info("ugallery sitemap child maps=%s", len(child_urls))
                ...
                try:
                    ...
                    for url, lastmod in parse_url_entries(xml_bytes):
                        if "?" in url or "#" in url:
                            continue
                        normalized = normalize_url(url)
                        key = ugallery_entity_from_url(normalized)
                        if key is None:
                            continue
                        entity_type, entity_id = key
                        entries.append(
        ...
        if skipped_child_maps:
            LOGGER.warning(
                "ugallery sitemap skipped child maps=%s sample=%s",
                len(skipped_child_maps),
                skipped_child_maps[:5],
            )
        return entries
...
def fetch_phaidon_sitemap_entries(
    index_url: str = DEFAULT_PHAIDON_INDEX,
    *,
    concurrency: int = 2,
    client: httpx.Client | None = None,
    fetch_bytes: FetchBytesFn | None = None,
) -> list[SitemapEntry]:
    """Fetch Phaidon product URLs from default-locale Shopify product sitemaps."""
    owned_client = False
    http: httpx.Client | None = None
    active_fetch = fetch_bytes
    ...
    if active_fetch is None:
        ...
        http = client or httpx.Client(
            ...
            headers={
                "User-Agent": (
                    "Mozilla/5.0 (compatible; PhaidonSitemapFetcher/1.0; "
                    "+https://www.phaidon.com/sitemap.xml)"
                ),
                "Accept": "application/xml,text/xml,*/*",
            },
        )
    ...
    try:
        index_xml = active_fetch(index_url)
        # Preserve query strings on child locs (Shopify ?from=&to=).
        child_urls = filter_phaidon_child_sitemaps(parse_child_sitemap_locs(index_xml))
        LOGGER.info("phaidon sitemap child maps=%s concurrency=%s", len(child_urls), concurrency)
                ...
                try:
                    ...
                    for url, lastmod in parse_url_entries(xml_bytes):
                        if "?" in url or "#" in url:
                            continue
                        normalized = normalize_url(url)
                        key = phaidon_entity_from_url(normalized)
                        if key is None:
                            continue
                        entity_type, entity_id = key
                        entries.append(
        ...
        if skipped_child_maps:
            LOGGER.warning(
                "phaidon sitemap skipped child maps=%s sample=%s",
                len(skipped_child_maps),
                skipped_child_maps[:5],
            )
        return entries
...
def known_phaidon_keys_from_paths(paths: Iterable[Path]) -> set[tuple[str, str]]:
    """Load known Phaidon entity keys from URL list / JSONL files."""
    keys: set[tuple[str, str]] = set()
    for url in load_urls(paths):
        key = phaidon_entity_from_url(url)
        if key is not None:
            keys.add(key)
    return keys


def known_ugallery_keys_from_paths(paths: Iterable[Path]) -> set[tuple[str, str]]:
    """Load known UGallery entity keys from URL list / JSONL files."""
    keys: set[tuple[str, str]] = set()
    for url in load_urls(paths):
        key = ugallery_entity_from_url(url)
        if key is not None:
            keys.add(key)
    return keys
